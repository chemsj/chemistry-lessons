...
## 31. 단일 슬라이드 "사례 학습" 포맷 (chem-3-04에서 확립 · 권장)
여러 일상 사례(암모니아·헤모글로빈·탄산음료·해양·치아)를 **한 사례 = 한 슬라이드**로 통일한 포맷. 템플릿 골격(`<!-- 골격 · 사례 학습 단일 슬라이드 -->`) 복제.
- 구조: `<section class="slide single">` → ① **헤더행**(kicker + title + `.hgroup` 2박스) ② **grid 1fr 1fr**(좌 시뮬 카드 / 우 그래프+공유 컨트롤 카드) ③ **reveal 버튼** ④ **핵심정리 카드**(`.flowrow` 칩 + `.sumconc`).
- **헤더 2박스**: 1번 = 화학반응식(상태 `(g)(aq)(s)(l)` 반드시 표기) + `.hbadge`(평형 이름). 2번 = `.hgroup.fx`에 요인 `.qchip` 2개(또는 2번째 평형식).
- **좌 시뮬**: `<canvas>` + `.readout`(큰 값 + `.rdchip`) + `.qkverdict`(현재 상태에 **연동**되는 판정, 고정 임계값 금지 → §23·현상태 기반).
- **우 그래프+컨트롤**: `.graphbox`+canvas / **공유 컨트롤** = 프리셋 버튼(JS 렌더·활성 강조) + 변인 슬라이더 + `.slider.spd` 속도 + 재생·일시정지(`⏸/▶`) + ↺ 초기화. 속도는 **핵심 동역학 전체**에 곱해질 것(§30 변형: 표시만 바꾸면 "효과 없음"으로 느껴짐).
- 미터형 정량(평형 수득률·속도·실효 등 0~100)은 `.meters`(3열 가로 막대) 사용.
- 모든 상태(프리셋·슬라이더·펼침)에서 **넘침 0** 확인. single은 답변/펼침 시 소량 스크롤 허용.

## 32. 이온·입자 막대그래프는 "상대·모식"으로 (절대 농도 아님)
- 용액 속 여러 이온량을 막대로 보일 때, **절대 몰농도를 선형으로 그릴 수 없다**: [H⁺]는 pH 1당 ×10(로그), [Ca²⁺]≫[H⁺] 등 자릿수가 다름. → 라벨에 **"상대량(모식)"** 명시.
- **경향·비율은 이론과 일치**시킨다(터무니없는 차이 금지):
  - 물의 자동 이온화: **pH 7에서 H⁺=OH⁻**, 산성일수록 H⁺>OH⁻(강산성 OH⁻≈0). (Kw)
  - **자유 이온 vs 총 용해량 구분**(중요): 고체가 녹아 이온이 "방출"돼도, 그 이온이 H⁺와 반응해 **소모**되면 자유 농도는 **감소**한다. 예) 법랑질 용해 시 Ca²⁺는 증가(미소모), **PO₄³⁻·OH⁻는 H⁺가 소모(H⁺+PO₄³⁻→HPO₄²⁻, H⁺+OH⁻→H₂O)해 감소** — 이 소모가 르샤틀리에로 용해를 끌어당김. 텍스트("소모")와 막대·시뮬 방향이 **반드시 일치**해야 학생 혼란이 없다.
- 막대는 캔버스(예 `drawXxBars`)로 그려 pH 등 변인에 **실시간 연동**, 범례 역할도 겸하게 함.

## 33. 형성평가 4문항 — 2×2 그리드(single) + 상단 점수판
- 4문항을 2단 컬럼(좌2·우2)에 넣으면 **피드백 펼침 시 넘침**. → `slide single`에 **2×2 그리드**(`grid-template-columns:1fr 1fr`) + 상단 헤더에 `.scoreband`(점수판) 배치.
- 채점 `check(btn,correct,'fbid',msg)`는 슬라이드 내 `.choices` 수를 **total로 자동 집계** → `#scoreNum`/`#scoreMsg` 갱신(점수판 HTML만 넣으면 동작).
- 문항은 사례를 **고르게 포괄**(산업·생명·환경·건강), 한 문항에 두 사례를 억지로 합치지 말 것.

## 34. 명명된 법칙은 학생 수준에 맞춰 생략 (평형으로 설명)
- 교육과정에서 안 배우는 명명 법칙(예 **헨리 법칙**)은 **이름을 쓰지 않고** 평형·르샤틀리에로 설명한다: "기체 용해 ∝ 압력", "**용해는 발열**이라 온도↑면 용해도↓".
- 학생 화면(슬라이드)엔 미언급, 교사용 `출처.md`에는 근거로 적어둘 수 있음.
